...
- 轮询登录验证码（邮箱或短信）
- 自动确认 OAuth 同意页
- 把 localhost 回调提交到 CPA、SUB2API、Codex2API、Manager，或由扩展直接生成 Codex 认证 JSON
...
- 平台验证链路中，CPA / SUB2API / Codex2API / Manager / 内置 Codex JSON 都按当前 `platform-verify` 可见步骤上报；SUB2API 内容脚本请求额外携带 `visibleStep`，避免 Plus 第 13 步落回普通 Step 10。
...
- CPA / SUB2API 配置
- Codex2API 配置
- Manager 配置；内置 Codex JSON 来源不需要额外服务配置
...
- `panelMode` 当前表示 `cpa | sub2api | codex2api | builtin-codex | manager` 五个来源；贡献模式仍不是新的来源枚举
...
7. 当主流程进入步骤 7 时，后台改为调用公开接口 `POST https://apikey.qzz.io/oauth/api/start` 申请贡献登录地址，而不是去 CPA / SUB2API / Codex2API / 内置 Codex JSON / Manager 来源刷新 OAuth
...
1. 按当前来源刷新 OAuth 地址：
   ...
   - 内置 Codex JSON：扩展本地生成 Codex OAuth URL、`state` 和 PKCE `code_verifier / code_challenge`
   - Manager：通过 Manager RPC 生成 ChatGPT OAuth 授权链接
2. 打开最新 OAuth 链接
3. 根据统一账号标识选择登录身份：
   - 邮箱账号：沿用现有邮箱登录链路
...
- 贡献模式下，步骤 7 不再从 CPA / SUB2API / Codex2API / 内置 Codex JSON / Manager 来源刷新 OAuth，而是直接调用公开贡献接口 `/oauth/api/start`
...
2. 判断当前来源是 CPA、SUB2API、Codex2API、内置 Codex JSON 还是 Manager
3. CPA / SUB2API 打开相应后台；Codex2API、内置 Codex JSON 和 Manager 直接走协议分支，不打开后台页面
4. 提交回调地址、等价的授权码交换请求，或下载本轮 Codex 认证 JSON
...
- 步骤 7 使用 Codex CLI OAuth 参数生成授权 URL，`redirect_uri` 固定为 `http://localhost:1455/auth/callback`，并保存本轮 `builtinCodexOAuthState / builtinCodexCodeVerifier / builtinCodexCodeChallenge`
- 步骤 10 校验 callback `state` 后，用 `code + code_verifier` 请求 `https://auth.openai.com/oauth/token`
- 步骤 10 解析 `id_token` 中的 `email`、`chatgpt_account_id` 和 `chatgpt_plan_type`，生成 CLIProxyAPI 兼容认证 JSON，并通过 Chrome 下载系统落到浏览器 Downloads 的 `Codex-OAuth-JSON/` 子文件夹
- 文件名遵循 CLIProxyAPI Codex 凭据规则：`codex-邮箱.json`、`codex-邮箱-plan.json` 或 `codex-账号哈希前8位-邮箱-team.json`
- 这条来源不依赖 CPA、SUB2API、Codex2API、Manager，也不会向外部账号池写入记录
...
1. 读取总轮数与模式；sidepanel 默认总轮数为 `5`，在点击“自动”瞬间先冻结目标轮数，后台确认进入同一目标轮数前会忽略旧 active 状态里的过期 `autoRunTotalRuns`
