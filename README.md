...
- 对客查询页：`http://127.0.0.1:18769/client`
  ...
  - 支持一次粘贴多行账号，页面会自动识别 `@duck.com`、`@2925.com` 或其他邮箱并保存在当前浏览器
...
- 服务只监听本机 `127.0.0.1`
- IMAP 密码/授权码会保存在本地 SQLite 数据库中，默认数据库文件为 `scripts/qq_openai_helper.sqlite3`
- 可通过环境变量 `IMAP_OPENAI_HELPER_DB_PATH` 指定数据库文件路径，旧的 `QQ_OPENAI_HELPER_DB_PATH` 仍兼容
