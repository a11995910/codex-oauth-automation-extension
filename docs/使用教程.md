...
5. 查看最终账号记录
   自动流程跑完最后一步后，侧边栏日志区的 `记录` 按钮会展示本轮账号记录。
   成功记录会保留账号、绑定手机号、密码、完成时间和最终状态，可直接点击 `复制` 复制账号密码。
   如果启用了本地 helper，同一份快照也会同步到 `data/account-run-history.json`。
...
新版第十步会通过 CPA 管理接口提交回调，并校验本轮 OAuth `state`，接口失败或 `state` 不匹配会直接报错，不会继续显示成功。

如果仍然没有认证文件，通常是因为 `cpa` 部署所在环境无法访问 `OpenAI`。请先确认部署环境可以正常访问 `OpenAI`，然后重新从 OAuth 登录步骤开始执行。

如果不想启动 CPA，可在侧边栏 `来源` 选择 `内置 Codex JSON`。该来源会由扩展自己生成 Codex OAuth 链接、交换 token，并在第十步直接下载 CLIProxyAPI 兼容的 `codex-*.json` 文件；文件会固定落到浏览器 Downloads 的 `Codex-OAuth-JSON/` 子文件夹。它不会写入 CPA 服务目录，也不会创建 CPA 账号记录。
