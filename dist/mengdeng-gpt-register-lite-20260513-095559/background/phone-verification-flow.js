(function attachBackgroundPhoneVerification(root, factory) {
  ...
  function createPhoneVerificationHelpers(deps = {}) {
    const {
      ...
      chrome: chromeApi = (typeof chrome !== 'undefined' ? chrome : null),
    ...
    function isAddPhoneCannotSendCodeError(value) {
      const text = String(value || '').trim();
      if (!text) {
        return false;
      }
      return /unable\s+to\s+send\s+(?:a\s+)?(?:verification\s+)?code\s+to\s+(?:this\s+)?phone\s+number|cannot\s+send\s+(?:a\s+)?(?:verification\s+)?code\s+to\s+(?:this\s+)?phone\s+number|can't\s+send\s+(?:a\s+)?(?:verification\s+)?code\s+to\s+(?:this\s+)?phone\s+number|try\s+again\s+later\s+or\s+use\s+(?:a\s+)?(?:different|other)\s+(?:phone\s+)?number|无法向此电话号码发送验证码|无法向该电话号码发送验证码|无法向这个电话号码发送验证码|无法向此手机号发送验证码|无法向该手机号发送验证码|请稍后重试或使用其他号码|请稍后重试或使用其它号码/i.test(text);
    }
    ...
    function isRecoverableAddPhoneSubmitError(value) {
      ...
      return (
        ...
        || isAddPhoneCannotSendCodeError(text)
    ...
    function isPhoneVerificationPageUrl(url) {
      return /https:\/\/auth\.openai\.com\/phone-verification(?:[/?#]|$)/i.test(String(url || '').trim());
    }

    function isPhoneSubmitNavigationTransportError(value) {
      const text = String(value || '').trim();
      if (!text) {
        return false;
      }
      return /Content script on signup-page did not respond in \d+s|Receiving end does not exist|message channel is closed|message channel closed|port closed before a response was received|A listener indicated an asynchronous response|back\/forward cache|load failed|networkerror/i.test(text);
    }

    async function getCurrentTabUrl(tabId) {
      if (!Number.isInteger(tabId) || typeof chromeApi?.tabs?.get !== 'function') {
        return '';
      }
      try {
    ...
    async function recoverPhoneVerificationStateAfterSubmitError(tabId, submitErrorText) {
      if (!isPhoneSubmitNavigationTransportError(submitErrorText)) {
        return null;
      }
      ...
      if (snapshot?.phoneVerificationPage || isPhoneVerificationPageUrl(snapshot?.url)) {
        return {
          ...(snapshot || {}),
          addPhonePage: false,
      ...
      if (isPhoneVerificationPageUrl(tabUrl)) {
        return {
          url: tabUrl,
          addPhonePage: false,
          phoneVerificationPage: true,
        ...
        while (true) {
          ...
          if (pageState?.addPhonePage) {
            ...
            try {
              ...
              const recoveredPageState = await recoverPhoneVerificationStateAfterSubmitError(tabId, submitErrorText);
              if (recoveredPageState?.phoneVerificationPage) {
                submitResult = recoveredPageState;
                await addLog('步骤 9：手机号提交后已进入验证码页，继续等待短信验证码。', 'info');
              } else if (isRecoverableAddPhoneSubmitError(submitErrorText)) {
            ...
            if (submitResult.addPhoneRejected) {
              ...
              try {
                ...
                const retryErrorText = String(submitError?.message || submitError || 'unknown error');
                const recoveredPageState = await recoverPhoneVerificationStateAfterSubmitError(tabId, retryErrorText);
                if (recoveredPageState?.phoneVerificationPage) {
                  submitResult = recoveredPageState;
                  retrySubmitError = null;
                  await addLog('步骤 9：手机号重提后已进入验证码页，继续等待短信验证码。', 'info');
                }
              }
