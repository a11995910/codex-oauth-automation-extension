...
5. SUB2API 多分组创建账号。
6. OAuth 登录使用全新标签页。
7. 接码平台多平台适配：HeroSMS + 5sim + NexSMS + SMSBower。
...
补充：接码平台多平台适配是当前分支新增的本地自定义功能；真实 5sim、NexSMS、SMSBower API Key 只允许通过侧边栏输入并保存到本地配置，不写入仓库、测试或文档。
...
## 8. 接码平台多平台适配：HeroSMS + 5sim + NexSMS + SMSBower
...
- 新增 `5sim`、`NexSMS`、`SMSBower` 平台，下拉框在接码设置卡片中直接可见。
- 国家/地区列表、API Key、价格上限、余额、买号、查码、完成、取消、ban、复用均按当前 `phoneSmsProvider` 分发。当前侧边栏常用地区优先保留印度尼西亚、泰国、越南。
- HeroSMS 继续使用原 `heroSms*` 字段；5sim、NexSMS、SMSBower 分别使用独立字段，切换平台时不会互相覆盖草稿。
- SMSBower 复用 `handler_api.php` 兼容协议，默认接口为 `https://smsbower.page/stubs/handler_api.php`，OpenAI 服务码为 `dr`；官方文档见 `https://smsbower.app/cn/api?page=client`。
...
| `phone-sms/providers/registry.js` | 接码平台注册表，按 `phoneSmsProvider` 规范化 HeroSMS、5sim、NexSMS、SMSBower。 |
| `background.js` | 持久字段默认值、导入脚本、配置规范化、phone flow helper 注入。 |
| `background/phone-verification-flow.js` | Step 9 运行态按 activation.provider 分发后续查码/完成/取消/ban/reuse，SMSBower 走 HeroSMS 兼容协议分支。 |
| `sidepanel/sidepanel.html` | 接码平台下拉、平台顺序、余额按钮、5sim operator 输入。 |
| `sidepanel/sidepanel.js` | 平台切换、地区列表联动、价格/余额查询、配置收集与恢复。 |
| `tests/five-sim-provider.test.js` | 5sim provider 单测。 |
| `tests/phone-verification-flow.test.js` | 5sim、NexSMS、SMSBower Step 9 分发和拿号测试。 |
| `tests/sidepanel-phone-verification-settings.test.js` | 接码平台下拉和侧边栏联动测试。 |

### 关键状态字段

- 通用：
  - `phoneSmsProvider: "hero-sms" | "5sim" | "nexsms" | "smsbower"`
...
1. 不要把真实 5sim、NexSMS、SMSBower key 写入代码、测试或文档；只允许用户在侧边栏输入。
2. 上游同步时如果改了 Step 9 手机号验证，必须保留 `currentPhoneActivation.provider` 驱动的后续分发。
3. 上游同步时如果改了 sidepanel 接码区域，必须保留 `select-phone-sms-provider` 和 `select-phone-sms-provider-order`，且地区列表按平台加载：HeroSMS/SMSBower 数字国家 ID，5sim 字符串 country slug，NexSMS 数字国家 ID。
4. 5sim 产品码当前固定为 `openai`，operator 默认 `any`；价格上限为空时按价格目录最低可用价尝试。
5. SMSBower 的 OpenAI 服务码来自官方文档中的 `OpenAI (ChatGPT)` 项，当前默认为 `dr`，侧边栏会单独显示 `SMSBower / 服务代码` 输入并随请求透传为 `service`；常用国家码按官方文档为印度尼西亚 `6`、泰国 `52`、越南 `10`，如果官方调整需同步更新默认值和测试。
6. SMSBower 下 `指定档位` 是严格档位：填写后只会请求该 `maxPrice`，不会在 `NO_NUMBERS` 后继续加试价格目录中更高的档位，避免用户设定 `0.008` 时误买 `0.196` 档位；HeroSMS 仍保留同国家多档位递进策略。
...
node --test tests/background-account-history-settings.test.js tests/five-sim-provider.test.js tests/phone-verification-flow.test.js tests/sidepanel-phone-verification-settings.test.js
```
...
```bash
rg -n "phoneSmsProvider|select-phone-sms-provider|PhoneSmsFiveSimProvider|PhoneSmsProviderRegistry|fiveSim|nexSms|smsBower|smsbower" \
  background.js \
