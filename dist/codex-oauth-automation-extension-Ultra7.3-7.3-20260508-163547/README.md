...
- 支持 `Auto` 多轮运行
- 支持中途 `Stop`
- 支持通过日志区的 `记录` 按钮查看账号记录面板，同一轮的邮箱和手机号会合并显示，最终成功后记录本账号与密码，并支持复制账号密码
...
如果你使用 `DuckDuckGo @duck.com` 虚拟邮箱，并把邮件转发到 QQ 邮箱，可以启动本地查询页快速查验证码，避免每次手动打开 QQ 邮箱搜索。

准备内容：

- QQ 邮箱已开启 `IMAP/SMTP`
- QQ 邮箱设置里生成的 `IMAP 授权码`
- 当前 Duck 虚拟邮箱地址，例如 `xxx@duck.com`
...
```powershell
.\start-qq-openai-helper.bat
```

macOS：

```bash
chmod +x ./start-qq-openai-helper.command
./start-qq-openai-helper.command
```

也可以直接运行：

```bash
python3 scripts/qq_openai_code_helper.py --open
```
...
- 后台配置页：`http://127.0.0.1:18769/admin`
  - 填写 QQ 邮箱地址、IMAP 授权码、邮箱目录、扫描封数和轮询间隔
  - 保存后会写入本地 SQLite 数据库，服务重启后会自动加载，无需重复填写
  - 后台会优先扫描当天新邮件，并按邮箱目录和 UID 去重写入数据库
  - 可查看数据库中最近收取到的邮件记录，并按 `@duck.com` 虚拟邮箱筛选
- 对客查询页：`http://127.0.0.1:18769/client`
  - 面向客户展示为 `GPT小店验证码查询`
  - 预留小店介绍区和小店链接位置
  - 支持粘贴 `violin-dime-blot@duck.com-----1q@W#E¥R` 这类账号密码格式
  - 支持一次粘贴多行账号，页面会自动识别邮箱并保存在当前浏览器
  - 左侧可选择账号，右侧会按对应邮箱查询 GPT 登陆验证码信件列表
  - 支持一键复制验证码、邮箱、密码或整条邮箱/密码/验证码信息

如果只打开根地址 `http://127.0.0.1:18769/`，默认进入后台配置页。

安全说明：

- 服务只监听本机 `127.0.0.1`
- QQ 授权码会保存在本地 SQLite 数据库中，默认数据库文件为 `scripts/qq_openai_helper.sqlite3`
- 可通过环境变量 `QQ_OPENAI_HELPER_DB_PATH` 指定数据库文件路径
- 对客页保存的 GPT 账号密码只放在当前浏览器的 `localStorage`，不会提交给 Python 服务
- 关闭终端窗口后再次启动，会自动从数据库加载后台配置和历史验证码记录
...
- 保存本轮 localhost 回调地址和 CPA 返回的验证状态
- 自动关闭匹配 `http://localhost:1455/auth` 这一类前缀的 localhost 残留页面
- 如果 CPA 管理接口返回错误、超时、管理密钥缺失或 state 不匹配，当前步骤会失败，不会标记为认证成功
...
- 自定义密码只存在当前会话存储中
- Step 3 会把本轮账号与密码写入运行态 `accounts`；最终步骤成功后，还会把账号、手机号、密码、最终状态和完成时间写入持久化 `accountRunHistory`，可在侧边栏 `记录` 面板查看或复制
