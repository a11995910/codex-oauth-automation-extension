...
function normalizePanelMode(value = '') {
  const normalized = String(value || '').trim().toLowerCase();
  if (normalized === 'sub2api' || normalized === 'codex2api' || normalized === 'manager') {
    return normalized;
  }
  return 'cpa';
}
...
function updatePanelModeUI() {
  ...
  const useCpa = !useSub2Api && !useCodex2Api && !useManager;
  ...
  if (step9Btn) {
    step9Btn.textContent = useSub2Api
      ? 'SUB2API 回调验证'
      : (useCodex2Api ? 'Codex2API 回调验证' : (useManager ? 'Manager 回调验证' : 'CPA 回调验证'));
  }
}
