...
const rowManagerUrl = document.getElementById('row-manager-url');
const inputManagerUrl = document.getElementById('input-manager-url');
const rowManagerRpcToken = document.getElementById('row-manager-rpc-token');
const inputManagerRpcToken = document.getElementById('input-manager-rpc-token');
const rowManagerTags = document.getElementById('row-manager-tags');
const inputManagerTags = document.getElementById('input-manager-tags');
const rowManagerNote = document.getElementById('row-manager-note');
const inputManagerNote = document.getElementById('input-manager-note');
...
const rowCodePlatformBaseUrl = document.getElementById('row-code-platform-base-url');
const inputCodePlatformBaseUrl = document.getElementById('input-code-platform-base-url');
const rowCodePlatformApiKey = document.getElementById('row-code-platform-api-key');
const inputCodePlatformApiKey = document.getElementById('input-code-platform-api-key');
const btnToggleCodePlatformApiKey = document.getElementById('btn-toggle-code-platform-api-key');
...
const DEFAULT_CODE_PLATFORM_BASE_URL = 'https://code.youkeduo.site';
...
let currentAutoRun = {
  ...
  phase: 'idle',
  currentRun: 0,
  totalRuns: DEFAULT_AUTO_RUN_TOTAL_RUNS,
  attemptRun: 0,
  scheduledAt: null,
  countdownAt: null,
  countdownTitle: '',
...
const MAIL_PROVIDER_LOGIN_CONFIGS = {
  ...
  [CODE_PLATFORM_PROVIDER]: {
    label: '验证码平台',
    url: DEFAULT_CODE_PLATFORM_BASE_URL,
    buttonLabel: '打开平台',
  },
  '2925': {
    label: '2925 邮箱',
...
function getRunCountValue() {
  ...
  const parsed = parseInt(inputRunCount.value, 10);
  return Math.max(1, Number.isFinite(parsed) ? parsed : DEFAULT_AUTO_RUN_TOTAL_RUNS);
}
...
function normalizeCodePlatformBaseUrlValue(value = '') {
  const raw = String(value || '').trim() || DEFAULT_CODE_PLATFORM_BASE_URL;
  const candidate = /^[a-zA-Z][a-zA-Z\d+\-.]*:\/\//.test(raw) ? raw : `https://${raw}`;
  try {
    const parsed = new URL(candidate);
    parsed.hash = '';
    parsed.search = '';
    const pathname = parsed.pathname === '/' ? '' : parsed.pathname.replace(/\/+$/, '');
    return `${parsed.origin}${pathname}` || DEFAULT_CODE_PLATFORM_BASE_URL;
  } catch {
    return DEFAULT_CODE_PLATFORM_BASE_URL;
  }
}
...
function collectSettingsPayload() {
  ...
  const smsBowerServiceCodeValue = typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode
    ? normalizeNexSmsServiceCodeForPayload(inputSmsBowerServiceCode.value || latestState?.smsBowerServiceCode || defaultSmsBowerServiceCode)
    : normalizeNexSmsServiceCodeForPayload(latestState?.smsBowerServiceCode || defaultSmsBowerServiceCode);
  ...
  return {
    ...
    managerUrl: inputManagerUrl.value.trim(),
    managerRpcToken: inputManagerRpcToken.value.trim(),
    managerTags: inputManagerTags.value.trim(),
    managerNote: inputManagerNote.value.trim(),
    ...
    codePlatformBaseUrl: typeof normalizeCodePlatformBaseUrlValue === 'function'
      ? normalizeCodePlatformBaseUrlValue(typeof inputCodePlatformBaseUrl === 'undefined' ? '' : inputCodePlatformBaseUrl?.value)
      : String((typeof inputCodePlatformBaseUrl === 'undefined' ? '' : inputCodePlatformBaseUrl?.value) || (typeof DEFAULT_CODE_PLATFORM_BASE_URL === 'undefined' ? 'https://code.youkeduo.site' : DEFAULT_CODE_PLATFORM_BASE_URL) || '').trim(),
    codePlatformApiKey: String(typeof inputCodePlatformApiKey === 'undefined' ? '' : inputCodePlatformApiKey?.value || '').trim(),
...
function updateHeroSmsPlatformDisplay() {
  ...
  const serviceText = provider === PHONE_SMS_PROVIDER_SMSBOWER
    ? ` / OpenAI (${normalizeNexSmsServiceCodeValue((typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode ? inputSmsBowerServiceCode.value : '') || latestState?.smsBowerServiceCode || DEFAULT_SMS_BOWER_SERVICE_CODE)})`
    : ' / OpenAI';
...
async function previewHeroSmsPriceTiers() {
  ...
  for (const provider of providerOrder) {
    ...
    const heroCompatibleServiceCode = provider === smsBowerProviderValue
      ? normalizeNexSmsServiceCodeValue((typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode ? inputSmsBowerServiceCode.value : '') || latestState?.smsBowerServiceCode || DEFAULT_SMS_BOWER_SERVICE_CODE)
      : 'dr';
...
function updateSignupMethodUI(options = {}) {
  ...
  syncStepDefinitionsForMode({ signupMethod: selectedMethod });
  if (typeof syncSignupPhoneInputFromState === 'function') {
    syncSignupPhoneInputFromState(latestState);
  }
}
...
function applySettingsState(state) {
  ...
  selectPanelMode.value = normalizePanelMode(state?.panelMode || 'builtin-codex');
  ...
  inputManagerUrl.value = state?.managerUrl || 'http://localhost:48760';
  inputManagerRpcToken.value = state?.managerRpcToken || '';
  inputManagerTags.value = state?.managerTags || '';
  inputManagerNote.value = state?.managerNote || '';
  ...
  const codePlatformProviderValue = typeof CODE_PLATFORM_PROVIDER === 'undefined' ? 'code-platform' : CODE_PLATFORM_PROVIDER;
  const restoredMailProvider = ['qq', '2925', codePlatformProviderValue].includes(String(state?.mailProvider || '').trim())
    ? String(state?.mailProvider || 'qq').trim()
    : 'qq';
  ...
  if (typeof inputCodePlatformBaseUrl !== 'undefined' && inputCodePlatformBaseUrl) {
    inputCodePlatformBaseUrl.value = typeof normalizeCodePlatformBaseUrlValue === 'function'
      ? normalizeCodePlatformBaseUrlValue(state?.codePlatformBaseUrl)
      : String(state?.codePlatformBaseUrl || (typeof DEFAULT_CODE_PLATFORM_BASE_URL === 'undefined' ? 'https://code.youkeduo.site' : DEFAULT_CODE_PLATFORM_BASE_URL) || '').trim();
  }
  if (typeof inputCodePlatformApiKey !== 'undefined' && inputCodePlatformApiKey) {
    inputCodePlatformApiKey.value = state?.codePlatformApiKey || '';
  }
  ...
  if (inputHeroSmsApiKey) {
    inputHeroSmsApiKey.value = restoredPhoneSmsProvider === fiveSimProviderValue
      ? (state?.fiveSimApiKey || '')
      : (
  ...
  if (typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode) {
    inputSmsBowerServiceCode.value = typeof normalizeNexSmsServiceCodeValue === 'function'
      ? normalizeNexSmsServiceCodeValue(state?.smsBowerServiceCode || defaultSmsBowerServiceCode)
  ...
  if (typeof applyHeroSmsFallbackSelection === 'function') {
    const primaryCountry = restoredPhoneSmsProvider === fiveSimProviderValue
      ...
      : (
        restoredPhoneSmsProvider === smsBowerProviderValue
          ? {
            id: normalizeHeroSmsCountryId(state?.smsBowerCountryId, defaultSmsBowerCountryId),
            label: normalizeHeroSmsCountryLabel(state?.smsBowerCountryLabel || defaultSmsBowerCountryLabel),
          }
          : {
    ...
    const restoredCountryId = restoredPhoneSmsProvider === fiveSimProviderValue
      ...
      : (
        restoredPhoneSmsProvider === smsBowerProviderValue
          ? String(normalizeHeroSmsCountryId(state?.smsBowerCountryId, defaultSmsBowerCountryId))
...
function renderReleaseSnapshot(snapshot) {
  ...
  switch (snapshot?.status) {
    ...
    default: {
      ...
      extensionVersionMeta.textContent = snapshot?.errorMessage || '检查失败';
...
function getMailProviderLoginUrl(provider = selectMailProvider.value) {
  ...
  const codePlatformProviderValue = typeof CODE_PLATFORM_PROVIDER === 'undefined' ? 'code-platform' : CODE_PLATFORM_PROVIDER;
  if (String(provider || '').trim() === codePlatformProviderValue) {
    const fallbackBaseUrl = typeof DEFAULT_CODE_PLATFORM_BASE_URL === 'undefined'
      ? 'https://code.youkeduo.site'
      : DEFAULT_CODE_PLATFORM_BASE_URL;
    const rawBaseUrl = (typeof inputCodePlatformBaseUrl === 'undefined' ? '' : inputCodePlatformBaseUrl?.value)
      || latestState?.codePlatformBaseUrl
      || fallbackBaseUrl;
    return typeof normalizeCodePlatformBaseUrlValue === 'function'
      ? normalizeCodePlatformBaseUrlValue(rawBaseUrl)
      : String(rawBaseUrl || fallbackBaseUrl).trim();
  }
...
function updateMailProviderUI() {
  ...
  const codePlatformProviderValue = typeof CODE_PLATFORM_PROVIDER === 'undefined' ? 'code-platform' : CODE_PLATFORM_PROVIDER;
  const useCodePlatform = selectMailProvider.value === codePlatformProviderValue;
  ...
  if (typeof rowCodePlatformBaseUrl !== 'undefined' && rowCodePlatformBaseUrl) {
    rowCodePlatformBaseUrl.style.display = useCodePlatform ? '' : 'none';
  }
  if (typeof rowCodePlatformApiKey !== 'undefined' && rowCodePlatformApiKey) {
    rowCodePlatformApiKey.style.display = useCodePlatform ? '' : 'none';
  }
  ...
  if (autoHintText && useCodePlatform) {
    autoHintText.textContent = '验证码平台会按当前注册邮箱直接查询线上验证码，不再打开网页邮箱。';
  }
...
function updatePanelModeUI() {
  ...
  const useBuiltinCodex = panelMode === 'builtin-codex';
  const useManager = panelMode === 'manager';
  const useCpa = !useSub2Api && !useCodex2Api && !useBuiltinCodex && !useManager;
  ...
  rowManagerUrl.style.display = useManager ? '' : 'none';
  rowManagerRpcToken.style.display = useManager ? '' : 'none';
  rowManagerTags.style.display = useManager ? '' : 'none';
  rowManagerNote.style.display = useManager ? '' : 'none';

  const step9Btn = document.querySelector('.step-btn[data-step-key="platform-verify"]');
  if (step9Btn) {
    step9Btn.textContent = useSub2Api
      ? 'SUB2API 回调验证'
      : (useCodex2Api ? 'Codex2API 回调验证' : (useBuiltinCodex ? '生成 Codex JSON' : (useManager ? 'Manager 回调验证' : 'CPA 回调验证')));
  }
}
...
[inputHotmailRemoteBaseUrl, inputHotmailLocalBaseUrl].forEach((input) => {
  input?.addEventListener('input', () => {
    markSettingsDirty(true);
    if (input === inputCodePlatformBaseUrl) {
      updateMailLoginButtonState();
    }
    scheduleSettingsAutoSave();
  });
...
[inputCodePlatformBaseUrl, inputCodePlatformApiKey].forEach((input) => {
  input?.addEventListener('input', () => {
    markSettingsDirty(true);
    scheduleSettingsAutoSave();
  });
  input?.addEventListener('blur', () => {
    if (input === inputCodePlatformBaseUrl) {
      input.value = normalizeCodePlatformBaseUrlValue(input.value);
    }
    saveSettings({ silent: true }).catch(() => { });
  });
});
...
selectPanelMode.addEventListener('change', async () => {
  const previousPanelMode = normalizePanelMode(latestState?.panelMode || 'builtin-codex');
...
async function switchPhoneSmsProvider(nextProvider) {
  ...
  if (previousProvider === fiveSimProviderValue) {
    ...
    patch.smsBowerServiceCode = normalizeNexSmsServiceCodeValue((typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode ? inputSmsBowerServiceCode.value : '') || latestState?.smsBowerServiceCode || DEFAULT_SMS_BOWER_SERVICE_CODE);
  } else {
  ...
  if (typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode) {
    inputSmsBowerServiceCode.value = normalizeNexSmsServiceCodeValue(latestState?.smsBowerServiceCode || DEFAULT_SMS_BOWER_SERVICE_CODE);
  }
...
if (typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode) {
  ...
  inputSmsBowerServiceCode.addEventListener('blur', () => {
    inputSmsBowerServiceCode.value = normalizeNexSmsServiceCodeValue(inputSmsBowerServiceCode.value || DEFAULT_SMS_BOWER_SERVICE_CODE);
    updateHeroSmsPlatformDisplay();
    saveSettings({ silent: true }).catch(() => { });
  });
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'BUILTIN_CODEX_AUTH_JSON_READY': {
      ...
      if (fileName && fileContent) {
        ...
        showToast(`Codex 认证 JSON 已生成：${fileName}`, 'success', 2200);
        sendResponse({ ok: true });
      } else {
        sendResponse({ error: 'Codex 认证 JSON 下载内容缺失。' });
      }
      return true;
    }
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.panelMode !== undefined) {
        selectPanelMode.value = normalizePanelMode(message.payload.panelMode || 'builtin-codex');
        updatePanelModeUI();
      }
      if (
      ...
      if (message.payload.codePlatformBaseUrl !== undefined && inputCodePlatformBaseUrl) {
        inputCodePlatformBaseUrl.value = normalizeCodePlatformBaseUrlValue(message.payload.codePlatformBaseUrl);
      }
      if (message.payload.codePlatformApiKey !== undefined && inputCodePlatformApiKey) {
        inputCodePlatformApiKey.value = message.payload.codePlatformApiKey || '';
      }
      ...
      if (message.payload.smsBowerServiceCode !== undefined && typeof inputSmsBowerServiceCode !== 'undefined' && inputSmsBowerServiceCode) {
