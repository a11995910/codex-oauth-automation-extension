  ...
  function createStep10Executor(deps = {}) {
    ...
    function getManagerRpcErrorMessage(payload, responseStatus = 500) {
      const details = [
        ...
        .map((value) => normalizeString(value))
        .find(Boolean);
      return details || `Manager RPC 请求失败（HTTP ${responseStatus}）。`;
    }
    ...
    function unwrapJsonRpcPayload(payload) {
      ...
      if (result && typeof result === 'object' && result.ok === false) {
        throw new Error(normalizeString(result.error || result.message) || 'Manager RPC 返回失败');
      }
      return result;
    }
    ...
    async function fetchManagerRpc(state, method, params = {}, options = {}) {
      ...
      const rpcToken = normalizeString(state.managerRpcToken);
      if (!rpcToken) {
    ...
    async function executeBuiltinCodexStep10(state) {
      const platformVerifyStep = resolvePlatformVerifyStep(state);
      const confirmOauthStep = resolveConfirmOauthStep(platformVerifyStep);
      const authLoginStep = resolveAuthLoginStep(platformVerifyStep);
      if (!builtinCodexAuth?.exchangeCodeForTokens || !builtinCodexAuth?.buildCodexAuthJson || !builtinCodexAuth?.downloadAuthJson) {
        throw new Error('内置 Codex OAuth 处理器未加载，请重新加载扩展后重试。');
      }
      if (state.localhostUrl && !isLocalhostOAuthCallbackUrl(state.localhostUrl)) {
        throw new Error(`步骤 ${confirmOauthStep} 捕获到的 localhost OAuth 回调地址无效，请重新执行步骤 ${confirmOauthStep}。`);
      }
      ...
      const callback = parseLocalhostCallback(state.localhostUrl, platformVerifyStep);
      const expectedState = normalizeString(state.builtinCodexOAuthState);
      if (!expectedState) {
        throw new Error(`缺少内置 Codex OAuth state，无法校验回调是否属于当前授权会话，请重新执行步骤 ${authLoginStep}。`);
      }
      if (expectedState !== callback.state) {
        throw new Error(`内置 Codex 回调 state 与当前授权会话不匹配，请重新执行步骤 ${authLoginStep}。`);
      }

      const codeVerifier = normalizeString(state.builtinCodexCodeVerifier);
      if (!codeVerifier) {
        throw new Error(`缺少内置 Codex OAuth code_verifier，请重新执行步骤 ${authLoginStep}。`);
      }

      await addStepLog(platformVerifyStep, '正在由扩展交换 Codex OAuth token...');
      const tokenResponse = await builtinCodexAuth.exchangeCodeForTokens(callback.code, codeVerifier, {
        redirectUri: `${new URL(callback.url).origin}/auth/callback`,
        timeoutMs: SUB2API_STEP9_RESPONSE_TIMEOUT_MS,
      });
      const { fileName, authJson, authInfo } = await builtinCodexAuth.buildCodexAuthJson(tokenResponse);

      await addStepLog(platformVerifyStep, `正在下载 Codex 认证 JSON：${fileName}`);
      await builtinCodexAuth.downloadAuthJson(fileName, authJson);

      const accountLabel = authInfo?.email || authInfo?.accountId || fileName;
      const verifiedStatus = `Codex 认证 JSON 已生成：${accountLabel}`;
      await addStepLog(platformVerifyStep, verifiedStatus, 'ok');
      await completeStepFromBackground(platformVerifyStep, {
        localhostUrl: callback.url,
        verifiedStatus,
        builtinCodexAuthFileName: fileName,
      });
    }

    async function executeManagerStep10(state) {
      const platformVerifyStep = resolvePlatformVerifyStep(state);
      const confirmOauthStep = resolveConfirmOauthStep(platformVerifyStep);
      const authLoginStep = resolveAuthLoginStep(platformVerifyStep);
      if (state.localhostUrl && !isLocalhostOAuthCallbackUrl(state.localhostUrl)) {
      ...
      if (!normalizeString(state.managerLoginId)) {
        throw new Error(`缺少 Manager 登录会话信息，请重新执行步骤 ${authLoginStep}。`);
      }

      const callback = parseLocalhostCallback(state.localhostUrl, platformVerifyStep);
      const expectedState = normalizeString(state.managerOAuthState || state.managerLoginId);
      if (expectedState && expectedState !== callback.state) {
        throw new Error(`Manager 回调 state 与当前授权会话不匹配，请重新执行步骤 ${authLoginStep}。`);
      }

      await addStepLog(platformVerifyStep, '正在向 Manager 提交回调并录入账号...');
      await fetchManagerRpc(state, 'account/login/complete', {
        state: callback.state,
        code: callback.code,
        redirectUri: `${new URL(callback.url).origin}/auth/callback`,
      }, {
      ...
      const verifiedStatus = 'Manager OAuth 账号录入成功';
      await addStepLog(platformVerifyStep, verifiedStatus, 'ok');
      await completeStepFromBackground(platformVerifyStep, {
        localhostUrl: callback.url,
        verifiedStatus,
