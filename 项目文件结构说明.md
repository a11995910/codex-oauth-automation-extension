...
- `background/panel-bridge.js`：来源桥接层；CPA 优先通过管理接口生成 OAuth 地址，若接口网络失败会自动打开 CPA 面板并走页面点击兜底，SUB2API 继续封装页面打开、脚本注入和通信，Codex2API / Manager 则直接通过后台协议生成 OAuth 地址；后台直连网络失败会输出带目标地址与原因的明确错误。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  码或从 QQ / 网易 / Gmail 转发目标邮箱收码；SMSBower 接码配置会单独显示服务代码输入，默认 `dr` 对应 OpenAI (ChatGPT)，用于透传给官方 `service` 参数；来源下拉框当前支持 `CPA / SUB2API / Codex2API`，其中 Codex2API 额外提供后台地址和管理密
